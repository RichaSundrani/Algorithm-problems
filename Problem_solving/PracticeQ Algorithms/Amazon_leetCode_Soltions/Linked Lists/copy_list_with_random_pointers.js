...
var noRandomClone = function (node, map) {
    if (node === null) return null;
    const res = new Node(node.val, noRandomClone(node.next, map), null);
    map.set(node, res);
    return res;
}
...
var copyRandomList = function (head) {
    const map = new Map();
    const newHead = noRandomClone(head, map);
    let current = head;
    let noRandomCurrent = newHead;
    while (current) {
        const random = current.random ? map.get(current.random) : null;
        noRandomCurrent.random = random;
        noRandomCurrent = noRandomCurrent.next;
